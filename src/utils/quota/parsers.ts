...
import type { ClaudeQuotaPayload, CodexUsagePayload, GeminiCliQuotaPayload } from '@/types';
...
export function parseClaudeQuotaPayload(payload: unknown): ClaudeQuotaPayload | null {
  if (payload === undefined || payload === null) return null;
  if (typeof payload === 'string') {
    const trimmed = payload.trim();
    if (!trimmed) return null;
    try {
      return JSON.parse(trimmed) as ClaudeQuotaPayload;
    } catch {
      return null;
    }
  }
  if (typeof payload === 'object') {
    return payload as ClaudeQuotaPayload;
  }
  return null;
}
